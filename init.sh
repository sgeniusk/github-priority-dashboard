...
echo "▶ JSON 무결성"
fail=0
for f in projects.json suggestions.json usage.json codex-summary.json codex-metrics.config.json history.json activity.json reports.json news.json project-logs.json monthly-analysis.json; do
  if [ -f "$f" ]; then
...
echo "▶ 핵심 파일 존재"
for f in dashboard.html dashboard.css monthly-analysis.html scripts/dashboard.js scripts/collect-codex-metrics.mjs scripts/build-dashboard.mjs scripts/prepare-sites-public.mjs scripts/refresh-progress.mjs scripts/build-monthly-analysis.mjs package.json .openai/hosting.json CLAUDE.md projects.schema.md feature_list.json progress.md; do
  [ -f "$f" ] && ok "$f" || warn "$f 없음"
done
