...
스크립트
- `scripts/refresh-progress.mjs` — GitHub 활동 수집 → `projects.json`(commits·lastUpdate·firstCommit·daysActive·meta.asOf) + `history.json` + `activity.json` 갱신, 그리고 `report-gen.mjs`로 `news.json`·`project-logs.json`까지 생성. `--dry-run`은 저장 없이 미리보기.
...
4. 브라우저 — `npx serve -l 4180` 후 `http://localhost:4180/dashboard.html`·`report.html`·`project-report.html?repo=habit` 열어 **전 탭/페이지 콘솔 에러 0** 확인.
