...
- `dashboard.html` — 메인. 4개 보기(운영실·프로젝트·Codex 작업·리소스) + `보고서`·`30일 분석` 링크. 수동 완성도는 참고값으로 내리고 실제 Codex 토큰·세션·최종 활동·예측 신뢰도를 우선 표시한다. `scripts/build-dashboard.mjs`가 생성한다.
...
- `codex-summary.json` — 공개 가능한 Codex root 세션 반올림 집계. 프로젝트별 누적·30일 토큰·세션·최근 활동·초기 예측.
...
- `scripts/collect-codex-metrics.mjs` — 로컬 Codex JSONL에서 root 세션의 마지막 누적 token_count만 집계. exact 원장은 `.codex-local/`에만 저장하고 공개 집계와 dashboard FALLBACK을 갱신한다.
- `scripts/build-dashboard.mjs` — projects·suggestions·usage·codex-summary에서 dashboard.html과 file:// FALLBACK 4종을 생성한다.
- `scripts/prepare-sites-public.mjs` — 기존 정적 뷰·JSON·프로젝트 페이지를 Sites 빌드용 임시 공개 디렉터리에 복제한다.
- `scripts/dashboard.js`·`dashboard.css` — 대시보드 동작과 스타일.
...
- `init.sh` — 전체 점검 진입점.
- `package.json`·`.openai/hosting.json` — 기존 정적 구조를 보존한 Codex Sites 빌드·배포 진입점.
...
### 운영 신호
대시보드는 `progress.total`을 프로젝트 카드의 작은 수동 참고값으로만 표시한다. 주 지표는 Codex 토큰·root 세션·최근 활동이며 토큰을 완성률로 부르지 않는다. 유한 프로젝트의 P50·P80은 개인 평균 root 세션 토큰과 규모 등급을 곱한 `low` 신뢰도 초기 예측이다. refresh는 커밋·날짜·히스토리만 갱신하며 완성도 점수를 자동 변경하지 않는다.
...
- `dashboard.html`: `fallback-projects`←projects.json · `fallback-suggestions`←suggestions.json · `fallback-usage`←usage.json · `fallback-codex`←codex-summary.json. `node scripts/build-dashboard.mjs`로 생성한다.
...
```
적용 후 `refresh-progress.mjs`가 `build-dashboard.mjs`를 호출해 dashboard FALLBACK을 함께 맞춘다. suggestions를 손으로 바꿨다면 `node scripts/build-dashboard.mjs`도 실행한다. 그다음 커밋·푸시.
...
3. `node scripts/build-dashboard.mjs`로 dashboard FALLBACK을, `project-report.html`의 `fallback-projects`/`fallback-reports` seed를 동기화.
...
Dashboard FALLBACK 동기화는 `node scripts/build-dashboard.mjs`를 사용한다. 프로젝트·제안·사용량·Codex 집계 중 하나가 바뀌면 이 명령으로 4개 seed를 함께 갱신한다. project-report.html의 `<script id="fallback-*">` seed는 해당 태그 안 JSON을 파일 내용으로 교체하고, report.html `FALLBACK_NEWS`는 기존 생성 절차를 따른다.

Sites 배포
```bash
npm run build
```
빌드 성공 후 `.openai/hosting.json`의 기존 프로젝트를 재사용해 Sites 버전을 저장·배포한다. 새 Sites 프로젝트를 중복 생성하지 않는다. GitHub Actions는 08:45·20:45 KST에 원격 활동을, Codex 로컬 자동화는 09:00·21:00 KST에 root 세션 토큰·사용량을 합쳐 갱신한다.
...
5. dashboard FALLBACK 4종과 보고서 FALLBACK이 JSON과 verbatim 일치하는지 확인.
...
- 이 저장소는 `main` 직접 푸시로 GitHub Pages에 배포되며, 같은 검증 상태를 Codex Sites에도 배포한다(브랜치 안 씀). 원격에 아침·저녁 자동 refresh 커밋이 올라오니 **푸시 전 `git fetch && git rebase origin/main`** 으로 정리한다. 충돌 시 `history.json`은 날짜 합집합으로, `projects.json`/`activity.json`은 최신(자기 쪽)으로.
