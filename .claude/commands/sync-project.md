...
## 4. 마무리 — blueprint.json 재생성

대시보드의 청사진 뷰는 루트의 `blueprint.json`(10개 `project.json` 집계본)을 읽는다. `project.json`을 바꿨으면 `blueprint.json`도 다시 만든다 — 모든 `projects/*/project.json`을 `projects.json`의 `rank` 순서로 모아 `{ generatedAt, projects:[...] }` 형태로 저장한다.
