...
- **`project.json`** — `repo`, `displayName`, `synced: true`, `versions[]` (+ 버전이 있으면 `currentVersion`). 각 버전은 `{id, label, status, target, summary, milestones[]}`.
...
- `versions[]`는 **원본에 실제 버전·로드맵 정보가 있을 때만** 채운다. 없으면 빈 배열로 둔다 — 임의 버전을 지어내지 않는다.
- `versions[].status`는 `done`/`inProgress`/`planned`. 완료 근거가 명확할 때만 `done`. `currentVersion`은 현재 `inProgress`인 버전의 `id`.
